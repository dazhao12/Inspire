...
output_folder_name <- "Acute_Status_3mo_first_nonMAC_3_30_2026"
...
diag_event_defs <- list(
  ...
  # Word pasted by user specifies I40-I41. This is kept as-is, although clinically
  # A40/A41 would usually align better with sepsis.
  list(var = "sepsis", label_cn = "脓毒症", codes = c("I40", "I41")),
...
for (def in diag_event_defs) {
  ...
  setnames(agg, c("flag", "interval_min", "event_chart_time", "source_code"),
...
for (def in ward_event_defs) {
  ...
  setnames(agg, c("flag", "interval_min", "event_chart_time", "source_value"),
...
summary_defs <- rbindlist(list(
  data.table(variable = vapply(diag_event_defs, `[[`, character(1), "var"),
             label_cn = vapply(diag_event_defs, `[[`, character(1), "label_cn"),
             source_type = "diagnosis"),
  data.table(variable = vapply(ward_event_defs, `[[`, character(1), "var"),
...
meta_notes <- data.table(
  note_type = c("window_definition", "anchor_definition", "sepsis_word_rule", "linkage_risk", "interval_rule"),
  note = c(
    ...
    "Sepsis kept exactly as Word pasted by user: ICD-10 I40-I41. This may be a clinical typo; A40-A41 would be a more common sepsis coding family.",

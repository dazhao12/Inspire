...
output_folder_name <- "Outcomes_word_complications_first_nonMAC_3_30_2026"
...
# ==============================================================================
# 2. Load diagnosis / labs / meds / ward_vitals
# ==============================================================================
...
labs <- labs[subject_id %in% anchor_index$subject_id]
labs[, `:=`(
  chart_time = as.numeric(chart_time),
  item_name = tolower(trimws(item_name)),
  value_num = suppressWarnings(as.numeric(value))
)]
labs <- labs[item_name == "creatinine" & !is.na(chart_time) & !is.na(value_num)]
...
aki_window <- anchor_index[, .(
  subject_id, op_id, orout_time,
  start = orout_time,
  end = postop7_end
)]
setkey(aki_window, subject_id, start, end)

closest_diag_event <- function(diag_matched, codes, event_name) {
  matched <- diag_matched[icd3 %chin% codes]
  if (nrow(matched) == 0L) {
    return(data.table(op_id = integer(), flag = integer(), interval = numeric()))
  }
  ...
  setnames(out, c("flag", "interval"), c(event_name, paste0(event_name, "_interval_after_surgery_min")))
  out
}
...
diag_events <- list(
  ...
  closest_diag_event(diag_matched, c("I40", "I41"), "sepsis")
)
...
make_med_flag <- function(matched_dt, pattern, event_name) {
  ...
  if (nrow(x) == 0L) return(data.table(op_id = integer(), flag = integer()))
...
ops_timeline <- anchor_index[, .(
  ...
  death_within_6month = as.integer(
    !is.na(allcause_death_time) & allcause_death_time >= orout_time & allcause_death_time - orout_time <= 129600
  ),
...
baseline_cr <- labs[
  anchor_index,
  on = .(subject_id, chart_time >= admission_time, chart_time < orin_time),
  allow.cartesian = TRUE,
  nomatch = 0L
][, .(baseline_creatinine = min(value_num, na.rm = TRUE)), by = op_id]
baseline_cr[is.infinite(baseline_creatinine), baseline_creatinine := NA_real_]

labs_postop <- labs[, `:=`(start = chart_time, end = chart_time)]
setkey(labs_postop, subject_id, start, end)
cr_postop <- foverlaps(
  labs_postop, aki_window,
  by.x = c("subject_id", "start", "end"),
  by.y = c("subject_id", "start", "end"),
  type = "within",
  nomatch = 0L
...
cr_postop <- merge(cr_postop, baseline_cr, by = "op_id", all.x = TRUE)
cr_postop <- cr_postop[!is.na(baseline_creatinine)]
if (nrow(cr_postop) > 0L) {
  cr_postop[, `:=`(
    ratio = value_num / baseline_creatinine,
    interval_min = chart_time - orout_time
  )]
  peak_cr <- cr_postop[order(op_id, -value_num, interval_min)][, .SD[1], by = op_id]
  peak_cr[, aki_creatinine_any := as.integer(ratio >= 1.5)]
  peak_cr[, aki_category := fifelse(
    ratio >= 3 | value_num >= 4, 3L,
    fifelse(ratio >= 2, 2L,
      fifelse(ratio >= 1.5, 1L, 0L)
    )
  )]
  peak_cr <- peak_cr[, .(
    op_id,
    aki_creatinine = aki_creatinine_any,
    aki_category = aki_category,
    peak_creatinine_postop = value_num,
    peak_creatinine_interval_after_surgery_min = interval_min
  )]
} else {
  peak_cr <- data.table(
    op_id = integer(),
    aki_creatinine = integer(),
    aki_category = integer(),
    peak_creatinine_postop = numeric(),
    peak_creatinine_interval_after_surgery_min = numeric()
  )
}
...
for (nm in c("aki_creatinine", "aki_category", "crrt_postop")) {
  if (!nm %in% names(aki_final)) aki_final[, (nm) := 0L]
  set(aki_final, which(is.na(aki_final[[nm]])), nm, 0L)
}
if (!"crrt_postop_interval_after_surgery_min" %in% names(aki_final)) {
  aki_final[, crrt_postop_interval_after_surgery_min := NA_real_]
}
aki_final[, aki_category := fifelse(crrt_postop == 1L & aki_category < 3L, 3L, aki_category)]
aki_final[, aki_creatinine := as.integer(aki_category >= 1L)]
...
final_dt <- copy(anchor_index[, .(subject_id, hadm_id, op_id, case_id, admission_time, orin_time, orout_time, discharge_time)])
...
flag_cols <- c(
  ...
  "death_within_6month", "unexpected_icu_admission_from_general_ward", "reoperation",
  "icu_stay", "aki_creatinine", "crrt_postop"
)
...
fwrite(final_dt, file.path(path_output, "postop_complications_word_defined_first_nonMAC.csv"))
...
summary_vars <- c(
  "death_within_hospital_stay", "death_within_30_days", "death_within_6month",
  ...
  "acute_kidney_failure_icd10", "aki_creatinine", "crrt_postop", "shock", "dic",
...
summary_out <- rbindlist(lapply(summary_vars, function(v) {
  interval_var <- switch(
    ...
    death_within_6month = "death_time_from_orout_min",
...
notes <- data.table(
  note_type = c("anchor_definition", "window_definition", "new_onset_af_rule", "sepsis_word_rule", "linkage_risk", "aki_rule"),
  note = c(
    "If an admission has multiple surgeries, anchor op is the first non-MAC surgery.",
    "Complication window = [orout_time, orout_time + 30 days] unless the metric itself specifies 7 days or 6 months.",
    "New-onset AF requires postop I48 and no preop I48 before orin_time.",
    "Sepsis is implemented exactly as Word pasted by user: ICD-10 I40-I41.",
    ...
    "AKI(creatinine) uses peak creatinine within 7 days after orout_time divided by baseline creatinine from admission to before orin_time; stage 3 is also triggered by postop CRRT."
  )
)

...
processed_dir <- "/N/project/analgesia_perioperation/data/INSPIRE_1.3/processed/Postop_Lab_Attributable_Window_Coverage_3_31_2026"
...
coverage <- fread(file.path(processed_dir, "postop_lab_attributable_window_coverage_summary.csv"))
item_cov <- fread(file.path(processed_dir, "postop_lab_attributable_window_item_coverage.csv"))
...
window_label_map <- c(
  current_stay_postop = "Current stay",
  attributable_postop_7d = "Attributable 7d",
  attributable_postop_15d = "Attributable 15d",
  attributable_postop_30d = "Attributable 30d",
  attributable_postop_60d = "Attributable 60d",
  cumulative_postop = "Cumulative postop"
)
...
p1 <- ggplot(overall_long, aes(x = window_label, y = coverage_pct, fill = metric)) +
  ...
  labs(
    title = "Postoperative Lab Coverage Across Candidate Windows",
    subtitle = "Overall coverage and key markers under different postop window definitions",
    x = NULL,
    y = "Coverage",
    fill = NULL
  ) +
...
ggsave(
  filename = file.path(figure_dir, "figure_postop_lab_window_coverage_overall.png"),
  plot = p1,
  width = 12,
  height = 6.5,
  dpi = 320
...
selected_items <- c("hb", "creatinine", "wbc", "albumin", "crp")
...
p2 <- ggplot(selected_dt, aes(x = window_label, y = pct_ops_with_item, color = item_label, group = item_label)) +
  ...
  labs(
    title = "Coverage of Representative Postop Labs by Window",
    subtitle = "Coverage expands as follow-up time widens beyond the index admission",
    x = NULL,
    y = "Coverage",
    color = NULL
  ) +
...
ggsave(
  filename = file.path(figure_dir, "figure_postop_lab_window_selected_items.png"),
  plot = p2,
  width = 12,
  height = 6.5,
  dpi = 320
...
top20_items <- item_cov[window == "attributable_postop_30d"][order(-pct_ops_with_item)][1:20, item_name]
heatmap_dt <- item_cov[window %in% window_order[1:5] & item_name %in% top20_items]
heatmap_dt[, item_name := factor(item_name, levels = rev(top20_items))]
...
p3 <- ggplot(heatmap_dt, aes(x = window_label, y = item_name, fill = pct_ops_with_item)) +
  ...
  labs(
    title = "Top 20 Postop Lab Items: Coverage Heatmap by Window",
    subtitle = "Restricted to current-stay and attributable postop windows",
    x = NULL,
    y = NULL,
    fill = "Coverage"
  ) +
...
ggsave(
  filename = file.path(figure_dir, "figure_postop_lab_window_heatmap_top20.png"),
  plot = p3,
  width = 11.5,
  height = 9.5,
  dpi = 320

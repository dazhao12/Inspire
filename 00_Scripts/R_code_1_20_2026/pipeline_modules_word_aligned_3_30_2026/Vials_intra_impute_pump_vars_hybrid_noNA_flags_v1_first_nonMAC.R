...
output_file <- file.path(processed_path, "vital_intraop_pump_vars_only_exposed_imputed_hybrid_noNA_with_flags.csv")
summary_file <- file.path(processed_path, "vital_intraop_pump_vars_only_exposed_imputed_hybrid_noNA_with_flags_summary.csv")
case_qc_file <- file.path(processed_path, "vital_intraop_pump_vars_only_exposed_imputed_hybrid_noNA_with_flags_case_qc.csv")
threshold_file <- file.path(processed_path, "vital_intraop_pump_vars_only_exposed_imputed_hybrid_noNA_with_flags_thresholds.csv")
note_file <- file.path(processed_path, "vital_intraop_pump_vars_only_exposed_imputed_hybrid_noNA_with_flags_note.md")
...
thresholds <- data.table(
  ...
  strategy = c(
    rep("Before first positive = 0; explicit 0 stays 0; positive values use 60-min capped LOCF; after cap expiry, fill 0 with imputation flag.", length(vaso_vars)),
    rep("Before first positive = 0; explicit 0 stays 0; target concentrations carry forward to OR end; no final NA.", length(anes_vars))
  )
)

impute_series_hybrid_no_na <- function(x, t, cap_minutes, orout_minus10) {
  n <- length(x)
  out <- rep(0, n)
  flag <- rep(1L, n)
  ...
  for (i in seq_len(n)) {
    ...
    if (!is.na(last_obs_val) && last_obs_val == 0) {
      out[i] <- 0
      flag[i] <- 1L
      state[i] <- "imputed_carry_zero_after_observed_zero"
      next
    }

    if (!is.na(last_obs_val) && last_obs_val > 0) {
      within_cap <- is.infinite(cap_minutes) || (!is.na(last_obs_time) && (ti - last_obs_time) <= cap_minutes)
      if (within_cap) {
        out[i] <- last_obs_val
        flag[i] <- 1L
        state[i] <- "imputed_carry_positive_within_cap"
      } else {
        out[i] <- 0
        flag[i] <- 1L
        state[i] <- if (!is.na(orout_minus10) && ti > orout_minus10) {
          "imputed_zero_post_orout_minus10"
        } else {
          "imputed_zero_after_cap_expiry"
        }
      }
    }
  }
...
for (idx in seq_along(pump_vars)) {
  ...
  tmp[, c("imputed_value", "imputed_flag", "imputation_state") := {
    end_time <- max(min_from_entry, na.rm = TRUE)
    orout_minus10 <- end_time - 10
    res <- impute_series_hybrid_no_na(value, min_from_entry, cap_minutes = rule$cap_minutes, orout_minus10 = orout_minus10)
  ...
  case_qc_list[[idx]] <- tmp[, .(
    ...
    n_imputed_carry_positive_within_cap = sum(imputation_state == "imputed_carry_positive_within_cap"),
    n_imputed_carry_zero_after_observed_zero = sum(imputation_state == "imputed_carry_zero_after_observed_zero"),
    n_imputed_zero_after_cap_expiry = sum(imputation_state == "imputed_zero_after_cap_expiry"),
    n_imputed_zero_post_orout_minus10 = sum(imputation_state == "imputed_zero_post_orout_minus10")
  ), by = op_id]
...
note_lines <- c(
  "# Pump Variable Hybrid No-NA Imputation With Flags",
  "",
  "This table removes all final NA values and adds per-variable imputation flags.",
  "",
  "Rules:",
  "- Apply relaxed threshold cleaning first.",
  "- Values failing thresholds are set to NA before filling.",
  "- Before first positive value in a case, fill with 0.",
  "- Observed 0 remains 0.",
  "- Positive values are carried forward within a 60-minute cap for vasoactive drugs.",
  "- After the cap expires, missing timestamps are conservatively filled with 0 instead of NA.",
  "- For ppfi and rfti, positive values carry forward to OR end.",
  "",
  "Per-variable companion columns `<var>_imputed_flag`:",
  "- 0 = observed value after threshold cleaning",
  "- 1 = imputed value",
  "",
  "Important:",
  "- This version is suitable for models that require no missing values.",
  "- Conservative zero fill after cap expiry should be interpreted together with the imputed flags."
)
writeLines(note_lines, note_file)

cat("Hybrid no-NA imputed file written to:\n", output_file, "\n", sep = "")

...
anchor_map <- anchor_ops[, .(
  ...
  antype, asa, age, sex, weight = weight_clean, height = height_clean, BMI
)]
...
diag[, `:=`(
  ...
  heart_failure = as.integer(icd3 == "I50"),
...
hb_index <- anchor_index[, .(
  subject_id,
  anchor_op_id = op_id,
  anchor_orin_time = orin_time,
  anchor_start_time = orin_time - 90 * 24 * 60
)]
hb_join <- labs_hb[hb_index, on = .(subject_id), allow.cartesian = TRUE, nomatch = 0L]
...
if (nrow(hb_join) > 0L) {
  ...
  hb_agg <- data.table(op_id = integer(), hb_n_preop = integer(), hb_last_g_dl = numeric(), hb_min_g_dl = numeric())
}
...
cr_join <- labs_cr[cr_window_index, on = .(subject_id), allow.cartesian = TRUE, nomatch = 0L]
...
if (nrow(cr_join) > 0L) {
  ...
  cr_agg <- data.table(
    op_id = integer(),
    creatinine_90d_n = integer(),
    creatinine_last_mg_dl = numeric(),
    creatinine_recent_1to2_mean_90d_mg_dl = numeric()
  )
}
...
final_dt <- merge(final_dt, cr_agg[, .(op_id, creatinine_90d_n, creatinine_last_mg_dl, creatinine_recent_1to2_mean_90d_mg_dl)], by = "op_id", all.x = TRUE)
...
keep_cols <- c(
  ...
  "atrial_fibrillation_flutter", "peripheral_vascular_disease",
  "cerebrovascular_disease", "dementia", "parkinsonism", "copd", "asthma",
  ...
  "connective_tissue_disease", "malignancy",
  "hb_last_g_dl", "hb_min_g_dl", "creatinine_last_mg_dl", "creatinine_recent_1to2_mean_90d_mg_dl",

...
ops_cleaned <- ops_raw %>%
  mutate(
    ...
    height_clean = case_when(
      !is.na(age) & age >= 18 & !is.na(height_clean) & (height_clean < 100 | height_clean > 250) ~ NA_real_,
      !is.na(age) & age < 18 & !is.na(height_clean) & (height_clean < 40 | height_clean > 250) ~ NA_real_,
      TRUE ~ height_clean
    ),
    weight_clean = case_when(
      !is.na(age) & age >= 18 & !is.na(weight_clean) & (weight_clean < 30 | weight_clean > 300) ~ NA_real_,
      !is.na(age) & age < 18 & !is.na(weight_clean) & (weight_clean < 1.5 | weight_clean > 200) ~ NA_real_,
      TRUE ~ weight_clean
    ),
    BMI = if_else(!is.na(height_clean) & height_clean > 0 & !is.na(weight_clean), weight_clean / ((height_clean / 100)^2), NA_real_),
    BMI = if_else(!is.na(BMI) & (BMI < 10 | BMI > 100), NA_real_, BMI),
    Age = age,
    Height = height_clean,
    Weight = weight_clean,
    Emergency_op = emop,
...
demog_subject_level <- ops_anchor %>%
  ...
  summarise(
    Age = mean_or_na(Age),
    Height = mean_or_na(Height),
    Weight = mean_or_na(Weight),
    BMI = mean_or_na(BMI),

...
# ==============================================================================
# 3. 特征工程：基于 Elixhauser 定义 (适配 3 位 ICD 代码)
# ==============================================================================
cat("正在计算 Elixhauser 并发症标志 (使用 3 位代码适配)...\n")
cat("正在补充 E14 系列代码...\n")
...
# --- 定义 Code Lists (源自 comorbidity.html + 用户补充) ---
...
# 注意：E14 (Unspecified) 的子类既包含单纯也包含复杂。但在 3 位截断逻辑下，所有 E14 都会变成 "E14"。
# 如果将 E14 同时放入 Uncomp 和 Comp 列表，那么：
#   diab_uncomp 标记 E14 = 1
#   diab_comp 标记 E14 = 1
#   最后清洗规则 (if Comp=1 -> Uncomp=0) 实际上会将所有 E14 归类为 "Complicated"。
#   这在逻辑上是 "Worst case assumption" (如果有 E14，可能是严重也可能是轻微，但在层级规则下被视为严重)。
#   如果用户想区分，在只有3位代码的情况下是不可能的。E14 就是 E14。
...
dt_merged[, `:=`(
  ...
  aids = check_codes_3digit(icd_clean, codes_aids),
...
# 保存
file_name <- "Diagnosis_Preop_Comorbidities_Elixhauser_3digit.csv"
...
# 映射中文名称标签 (Optionally add English/Chinese Labels if needed)
# 这里仅保存原始变量名

summary_file <- file.path(path_output, "Summary_Elixhauser_Prevalence_3digit.csv")
